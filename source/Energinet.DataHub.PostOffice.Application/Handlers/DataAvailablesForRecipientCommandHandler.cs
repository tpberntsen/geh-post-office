...
{
    public class DataAvailablesForRecipientCommandHandler : IRequestHandler<DataAvailableNotificationsForRecipientCommand, DataAvailableNotificationResponse>
    {
        ...
        private async Task ProcessGroupAsync(IEnumerable<DataAvailableNotificationCommand> group, CabinetKey key)
        {
            var dataAvailableNotifications = group.Select(Map);
            await _dataAvailableNotificationRepository.SaveAsync(dataAvailableNotifications, key).ConfigureAwait(false);
        }
    }
}
