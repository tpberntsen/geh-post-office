...
module "azfun_marketoperator_dequeue" {
  source                                    = "git::https://github.com/Energinet-DataHub/geh-terraform-modules.git//function-app?ref=1.8.0"
  name                                      = "azfun-marketoperator-dequeue-${var.project}-${var.organisation}-${var.environment}"
  resource_group_name                       = data.azurerm_resource_group.postoffice.name
  location                                  = data.azurerm_resource_group.postoffice.location
  storage_account_access_key                = module.azfun_marketoperator_dequeue_stor.primary_access_key
  storage_account_name                      = module.azfun_marketoperator_dequeue_stor.name
  app_service_plan_id                       = module.azfun_marketoperator_dequeue_plan.id
  ...
  dependencies                              = [
    ...
    module.azfun_marketoperator_dequeue_stor.dependent_on,
    module.stor_marketoperator_response.dependent_on,
  ]
}

module "azfun_marketoperator_dequeue_plan" {
  source              = "git::https://github.com/Energinet-DataHub/geh-terraform-modules.git//app-service-plan?ref=1.8.0"
  name                = "asp-marketoperator-dequeue-${var.project}-${var.organisation}-${var.environment}"
  resource_group_name = data.azurerm_resource_group.postoffice.name
  location            = data.azurerm_resource_group.postoffice.location
  kind                = "FunctionApp"
  sku                 = {
...
resource "random_string" "marketoperator_dequeue" {
  length  = 10
  special = false
  upper   = false
}